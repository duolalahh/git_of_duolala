...
class loginForm(forms.Form):
    username_or_email=forms.CharField(label='用户名or邮箱',widget=forms.TextInput(attrs={'class':'form-control','placeholder':'请输入用户名or邮箱'}))
...
class RegForm(forms.Form):
    ...
    def clean(self):
        ...
        if not (code !='' and code == verification_code):
            raise forms.ValidationError('验证码写错了嗷~')
        return self.cleaned_data
...
class BindEmailForm(forms.Form):
    ...
    def clean(self):
        ...
        if self.request.user.email !='':
            raise forms.ValidationError('您已经绑定过了~')
        #判断验证码
    ...
    def clean_verification_code(self):
        ...
        if verification_code == '':
            raise forms.ValidationError('验证码不能为空')
        return clean_verification_code
...
class ChangePasswordForm(forms.Form):
    ...
    def clean_old_password(self):
        ...
        if not self.user.check_password(old_password):
            raise forms.ValidationError('旧密码输入不正确')
        return old_password
      
class ForgotPasswordFrom(forms.Form):
    """docstring for FOrgotPasswordFrom"""
    email = forms.EmailField(label='邮箱',
    ...
    def __init__(self,*args,**kwargs):
        if 'request' in kwargs:
            self.request = kwargs.pop ('request')
        super(ForgotPasswordFrom,self).__init__(*args,**kwargs)
