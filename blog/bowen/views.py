...
def get_blog_common_data(request,blogs_all_list):
    paginator = Paginator(blogs_all_list, settings.EACH_PAGE_NUMBER)
    page_num = request.GET.get('page',1) #得到页码参数
    page_of_blogs = paginator.get_page(page_num) 
    a = page_of_blogs.number #当前页
    page_range = list(range(max(a-2,1),a))+\
        list(range(a,min(a+2,paginator.num_pages)+1))
    #省略页码标记
    if page_range[0]-1>=2:
        page_range.insert(0,'...')
